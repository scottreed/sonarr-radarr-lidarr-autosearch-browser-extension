...
- 2026-09-25: Trakt engine rewritten for the app.trakt.tv SvelteKit redesign. Detail pages: icon prepended to `.trakt-summary-title h1`; IMDb links only exist in the details drawer (`?view=details`), so the search term is `imdb:tt…` when present, else the title (drawer toggles change the URL, so the SPA watcher re-runs and upgrades the link). Listings moved to `/discover/*` with mixed `?mode=show|movie|media`; cards are routed per card via `:has(a.trakt-link[href^="/shows/"|"/movies/"])` and the icon goes in `.trakt-card-action-bar`. The first `runEngines()` usually fires before the SPA renders, cards fill lazily on scroll, and navigation keeps the old page rendered ~2s before swapping, so trakt.js runs a Trakt-only MutationObserver (150ms debounce) that keys each title/card by its search term (`data-servarr-trakt-key`), clears stale icons when a key changes, and calls `runEngines()` (bare name, not `window.runEngines`, for Firefox sandbox safety). Engines have no `deferMs`; detail terms are skipped until `og:url` matches the URL.
